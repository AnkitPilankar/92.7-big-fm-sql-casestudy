...
Select hostname,sum(showcount) as total_show from hosts
join shows using(hostid)
group by hostname;
...
select hostname,dense_rank() over(order by showcount desc) as den_rnk,
rank() over(order by showcount desc) as rnk,
row_number() over(order by showcount desc) as row_num 
from hosts
